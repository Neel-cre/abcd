# -*- coding: utf-8 -*-
"""Assignment_EE798R_Ayush_210245.ipynb
...
# import tensorflow as tf
# from tensorflow.keras.layers import Input, Embedding, Bidirectional, LSTM, GRU, Dense, Dropout, TimeDistributed, GlobalAveragePooling1D
# from tensorflow.keras.preprocessing.sequence import pad_sequences
# from sklearn.model_selection import train_test_split
# import numpy as np
# import nltk
...
# def prepare_dialog_data(sentences, labels, dialog_size=5):
#     dialogs = []
#     dialog_labels = []
#     for i in range(0, len(sentences), dialog_size):
#         dialog = sentences[i:i + dialog_size]
#         if len(dialog) == dialog_size:
#             dialogs.append(dialog)
#             dialog_labels.append(max(labels[i:i + dialog_size], key=labels.count))  # Majority label for the dialog
#     return dialogs, dialog_labels

# # Step 2: Tokenize and Extract data from your dataset
# def extract_data_from_dataset(concatenated_dataset):
#     sentences = [example['text'] for example in concatenated_dataset]
#     labels = [example['label'] for example in concatenated_dataset]

#     # Tokenize the sentences
#     tokenized_sentences = [word_tokenize(sentence.lower()) for sentence in sentences]  # Lowercase and tokenize
#     return tokenized_sentences, labels

# # Load the concatenated dataset
# sentences, labels = extract_data_from_dataset(concatenated_dataset)

# # Prepare dialogs (group sentences into dialogs)
# dialogs, dialog_labels = prepare_dialog_data(sentences, labels, dialog_size=5)

# # Step 3: Build a simple word-to-index tokenizer (or you can use a pre-built one)
# def build_tokenizer(dialogs):
#     word_to_index = {}
#     index = 1  # Start from 1 because 0 is reserved for padding
#     for dialog in dialogs:
...
# word_to_index = build_tokenizer(dialogs)

# # Step 4: Convert dialogs to integer tokens using the tokenizer
# def convert_to_token_ids(dialogs, word_to_index):
#     tokenized_dialogs = []
#     for dialog in dialogs:
#         tokenized_dialog = [[word_to_index.get(word, 0) for word in sentence] for sentence in dialog]
#         tokenized_dialogs.append(tokenized_dialog)
#     return tokenized_dialogs

# tokenized_dialogs = convert_to_token_ids(dialogs, word_to_index)

# # Split into train, validation, and test sets
# train_dialogs, test_dialogs, train_labels, test_labels = train_test_split(tokenized_dialogs, dialog_labels, test_size=0.2)
# train_dialogs, val_dialogs, train_labels, val_labels = train_test_split(train_dialogs, train_labels, test_size=0.2)

# # Step 5: Pad the dialogs and sentences
# max_sentence_len = max([len(sentence) for dialog in tokenized_dialogs for sentence in dialog])  # Max words in a sentence
# max_dialog_len = 5  # Fixed number of sentences per dialog

# def pad_dialogs(dialogs, max_dialog_len, max_sentence_len):
#     return pad_sequences(
#         [pad_sequences(dialog, maxlen=max_sentence_len, padding='post') for dialog in dialogs],
#         maxlen=max_dialog_len, padding='post'
#     )
...
# def build_dialog_model(vocab_size, embedding_size, sentence_rnn_size, dialog_rnn_size, num_classes, dropout_rate=0.5):
#     # Input layers
#     dialog_input = Input(shape=(max_dialog_len, max_sentence_len), dtype='int32')  # (batch_size, num_sentences, num_words)

#     # Word-level Embedding
#     embedding_layer = Embedding(vocab_size, embedding_size, mask_zero=True)
#     word_embeddings = TimeDistributed(embedding_layer)(dialog_input)  # (batch_size, num_sentences, num_words, embedding_dim)

#     # Word-level BiLSTM
#     word_lstm = TimeDistributed(Bidirectional(LSTM(sentence_rnn_size, return_sequences=False)))(word_embeddings)
#     # Now shape: (batch_size, num_sentences, rnn_units)

#     # Dialog-level GRU
#     dialog_gru = GRU(dialog_rnn_size, return_sequences=True)(word_lstm)  # (batch_size, num_sentences, rnn_units)

#     # Self-Attention Layer (Optional)
#     attention = tf.keras.layers.Attention()([dialog_gru, dialog_gru])  # Self-attention over dialog
#     dialog_representation = GlobalAveragePooling1D()(attention)  # (batch_size, rnn_units)

#     # Output Layer
#     dropout = Dropout(dropout_rate)(dialog_representation)
#     output = Dense(num_classes, activation='softmax')(dropout)

#     # Build the model
#     model = tf.keras.Model(inputs=dialog_input, outputs=output)
#     model.compile(optimizer='adam', loss='sparse_categorical_crossentropy', metrics=['accuracy'])

#     return model

# # Step 7: Define Hyperparameters and Build the Model
# vocab_size = len(word_to_index) + 1  # Plus one for padding token
# embedding_size = 100
# sentence_rnn_size = 128
# dialog_rnn_size = 128
...
# model.fit(train_dialogs_padded, np.array(train_labels), validation_data=(val_dialogs_padded, np.array(val_labels)), epochs=5, batch_size=32)

# # Step 9: Evaluate the Model
# test_loss, test_acc = model.evaluate(test_dialogs_padded, np.array(test_labels))
